...
const Render = {
    ...
    drawCharacterBody(character, color) {
        ...
        // Damage blink (touch-limit HP penalty): quick flicker to show damage taken.
        if ((character.damageBlinkTimeLeft ?? 0) > 0 && (character.damageBlinkDuration ?? 0) > 0) {
            const t = 1 - (character.damageBlinkTimeLeft / character.damageBlinkDuration);
            const pulse = 0.5 + 0.5 * Math.sin(t * Math.PI * 8); // fast flicker
            const dmgAlpha = 0.35 + 0.65 * pulse;
            alpha *= dmgAlpha;
        }
    ...
    _drawBallBodyInternal(ball) {
        ...
        // Touch counter visual cue:
        // - Normal state: bright red
        // - Touches exhausted (0): slightly darker red
        ...
        // Apply rotation transform for visual effect
        ctx.save();
        ctx.translate(proj.x, proj.y);
        ctx.rotate(ball.rotationAngle ?? 0);
        
        // Draw main ball body
        ctx.fillStyle = isDark ? '#b00000' : '#ff0000';
        ctx.beginPath();
        ctx.arc(0, 0, finalBallSize, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Draw ball highlight (rotated with the ball)
        // Highlight also darkens slightly when exhausted.
        ctx.fillStyle = isDark ? '#d45555' : '#ff6666';
        ctx.beginPath();
        ctx.arc(-finalBallSize * 0.3, -finalBallSize * 0.3, finalBallSize * 0.4, 0, Math.PI * 2);
        ctx.fill();
